...
RUTA_code_with_sybsecurity="/home/coecogni/espacios/codes_with_sybsecurity.isql"
...
do
    ...
    portunn=`echo $line|awk -F\, '{print $12}'`
    ...
    if [ "$status1" == "FAIL" ];then
        ...
        echo "ERROR CREDENCIALES EN EL SISTEMA $sid DEL CLIENTE $cliente, PORFAVOR REVISAR"
            ...
            if [ "$portunn" == "1" ];then
                SSHPASS=$pass_unix sshpass -e $pass scp -pr $RUTA_code_with_sybsecurity $user_unix@$ip:$RUTE
                echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip << EOF
isql -S'$sid' -U'$userdb' -P'$passdb' -X -w2000 -i'$RUTE/codes_with_sybsecurity.isql' > $RUTE/output_file_sid;
EOF
            else
...
EOF
            ...
            if [ "$portunn" == "1" ];then
                ...
                DATABASE_SIZE_sybsystemdb=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 10 { print \$2 } ' ")
                DATABASE_USED_sybsystemdb=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 10 { print \$3 } ' ")
                DATABASE_FREE_sybsystemdb=$(awk '{print $1-$2}' <<<"$DATABASE_SIZE_sybsystemdb $DATABASE_USED_sybsystemdb");

                DATABASE_SIZE_sybsystemprocs=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 11 { print \$2 } ' ")
                DATABASE_USED_sybsystemprocs=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 11 { print \$3 } ' ")
                DATABASE_FREE_sybsystemprocs=$(awk '{print $1-$2}' <<<"$DATABASE_SIZE_sybsystemprocs $DATABASE_USED_sybsystemprocs");

                DATABASE_SIZE_sybsecurity=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 9 { print \$2 } ' ")
                DATABASE_USED_sybsecurity=$(echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " cat $RUTE/output_file_sid | awk ' FNR == 9 { print \$3 } ' ")
                DATABASE_FREE_sybsecurity=$(awk '{print $1-$2}' <<<"$DATABASE_SIZE_sybsecurity $DATABASE_USED_sybsecurity");
            ...
            if [ "$portunn" == "1" ];then
                DATABASE_USED_sybsecurity=$(awk '{print $1*1000}' <<<"$DATABASE_USED_sybsecurity");
                DATABASE_FREE_sybsecurity=$(awk '{print $1*1000}' <<<"$DATABASE_FREE_sybsecurity");
                DATABASE_SIZE_sybsecurity=$(awk '{print $1*1000}' <<<"$DATABASE_SIZE_sybsecurity");
            fi
            ...
            if [ "$portunn" == "1" ];then
                echo "SIZE sybsecurity : $DATABASE_SIZE_sybsecurity  USED : $DATABASE_USED_sybsecurity FREE: $DATABASE_FREE_sybsecurity"
            fi


            if [ "$portunn" == "1" ];then
                echo $line | SSHPASS=$pass_unix sshpass -e $pass ssh -o "StrictHostKeyChecking no" $user_unix@$ip " rm -f $RUTE/output_file_sid $RUTE/codes_with_sybsecurity.isql";
            else
            ...
            if [ "$portunn" == "1" ];then
                echo $line | echo $cliente","$typedb","$FECHA","$sid","$descripcion","SYBSECURITY","$DATABASE_SIZE_sybsecurity","$DATABASE_FREE_sybsecurity","$DATABASE_USED_sybsecurity >> /home/coecogni/espacios/todos_sybase
            fi
