...
def connect_db2():
	for (dbtype, customer, dbname, dbuser, dbpass, dbalias, sid, hostname, dbport, usertunn, hostunn, portunn, umbralert,ip) in data_sql_db2:
                ...
                if (dbname[:5] == 'AS400' and customer[:1] != '#'):
                        a=os.system('sh /home/coecogni/espacios/as400_space.sh'+' '+ip+' '+dbuser+' '+dbpass+' '+'"{}"'.format(customer)+' '+dbtype+' '+sid+' '+'"{}"'.format(dbname))
                        print(a)
                
                if (customer[:1] != '#' and dbname[:5] != 'AS400'):
                        try:
                ...
                # TODO make error handling for connection
                        ...
                        # Fetch first result
                                ...
                                # TODO whatever needed with the result
                                        ...
                                        stm = "INSERT INTO espacios_test values " + ','.join(inserts) + ";"
        ...
        for (dbtype, customer, dbname, dbuser, dbpass, dbalias, sid, hostname, dbport, usertunn, hostunn, portunn, umbralert,ip) in data_sql_ora:
                ...
                try:
                        if customer[:1]!='#':
                                inserts=[];
                                if portunn[:1]=='1':
                                        a=os.system('sh /home/coecogni/espacios/tunnel.sh'+' '+umbralert+' '+portunn+' '+hostname+' '+dbport+' '+usertunn+' '+hostunn)
                                ...
                                for i in data_split2:
                                        ...
                                        sql = "INSERT INTO espacios_test (CLIENTE,TIPO_DB,FECHA,SID,DESCRIPCION,TABLESPACE,TAM_KBYTES,LIBRE_KBYTES,OCUPADO_KBYTES,MAX_BLOQUE_LIBRE,CREC_OCUPADO_DIARIO_KBYTES,EXTENTS,CREC_DIARIO_EXTENTS,MAX_CRECI_ULTIMOS_DIAS) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        ...
        for (dbtype, customer, dbname, dbuser, dbpass, dbalias, sid, hostname, dbport, usertunn, hostunn, portunn, umbralert,ip) in data_sql_syb:
                if customer[:1]!='#':
                        with open("/home/coecogni/espacios/todos.sybase","w") as sybase:
                                sybase.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (dbtype,customer,dbname,dbuser,dbpass,dbalias,sid,hostname,ip,usertunn,umbralert))
                        try:
...
cur_sql_ora.execute ("select dbtype, customer, dbname, dbuser, dbpass, dbalias, sid, hostname, dbport, usertunn, hostunn, portunn, umbralert,ip from espacios_ini where dbtype='ORA'");
...
cur_sql_db2.execute ("select dbtype, customer, dbname, dbuser, dbpass, dbalias, sid, hostname, dbport, usertunn, hostunn, portunn, umbralert,ip from espacios_ini where customer='GECOLSA'");
